...
module.export("CKConverter", function (osu_file_contents, options) {
  ...
  if (options.time_signature === "") {
    options.time_signature = {
      numerator: beatmap.timingPoints[0].timingSignature,
      denominator: 4,
    };
  } else {
  ...
  for (var i = 0; i < beatmap.hitObjects.length; i++) {
    ...
    var measure = Math.floor(absoluteBeat / timeSigNumerator);
    //var beat = Math.round((currentObj.startTime / beatUnitInMs) * 100) / 100;

    var absoluteLane = hitobj_x_to_track_number(currentObj.position.get("x"));
    var lane = ((absoluteLane - 1) % 3) + 1; // Wrap lane value if it exceeds 3
    var trackState = absoluteLane > 3 ? 1 : 0; // Make state 1 if it had to wrap its value
    var noteType = "";
    var duration = "";
    ...
    if (noteType == "Hold") {
      duration =
        Math.round((currentObj.endTime / beatUnitInMs - beat) * 100) / 100;
      append_to_output(`,\n\t\t\t"duration": ${duration}`);
    }
